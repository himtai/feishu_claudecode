...
**工具调用过程** — Claude 执行任务时实时显示工具调用状态和表情回应：

![工具调用演示](assets/gif1.gif)

**交互式卡片回复** — 需要用户选择时自动发送带按钮的卡片：

![卡片回复演示](assets/gif2.gif)
...
在用户原消息上自动添加/切换表情，无需滚动页面即可感知执行状态：
...
- **💭 思考过程** — Claude 的推理内容（实时滚动）
- **⚙️ 当前操作** — 正在执行的工具及参数
- **⏱️ 运行时间** — 已执行时长
- **🔧 工具调用次数** — 累计工具调用数
- **🎯 Token 消耗** — 输入/输出 token 总量
- **📊 上下文占用** — 当前上下文窗口使用百分比（任务完成时显示）
